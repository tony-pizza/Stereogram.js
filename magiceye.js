...
(function () {
  var MagicEye = {
    render: function (opts) {
      ...
      if (opts.el) {
        ...
        if (!element || !element.tagName) {
          throw new Error('Could not find element: ' + opts.el);
        }
      }
      ...
      if (!width) {
        throw new Error('Width not set and could not be inferred from element: ' + opts.el);
      }
      if (!height) {
        throw new Error('Hight not set and could not be inferred from element: ' + opts.el);
      }
      ...
      if (opts.depthMap) {
        ...
        throw new Error('No depthMap or depthMapper opts given');
      }
      ...
      switch (element.tagName) {
        ...
        default:
          throw new Error('el must be either a <canvas> or an <img>');
      }
    ...
    generatePixelData: function (opts) {
      ...
      var numColors = opts.colors.length;
      var same; // points to a pixel to the right
      var dpi = 72; // assuming output of 72 dots per inch
      var eyeSep = Math.round(2.5 * dpi); // eye separation assumed to be 2.5 inches
      var mu = (1 / 3); // depth of field (fraction of viewing distance)
  ...
  DepthMapper.prototype.make = function () {
  ...
  DepthMapper.prototype.generate = function (width, height) {
    if (this.autoResize) {
      return this.resize(this.make(width, height), width, height);
    }
  ...
  DepthMapper.prototype.resize = function (origDepthMap, width, height) {
    var origDepthMapY;
    var resizedDepthMap = [];
    var origDepthMapHeight = origDepthMap.length;
    var origDepthMapWidth = origDepthMap[0].length;
    var x;
    var y;
    ...
    for (y = 0; y < height; y++) {
      ...
      for (x = 0; x < width; x++) {
        resizedDepthMap[y][x] =
          origDepthMap[origDepthMapY][Math.floor(x * origDepthMapWidth / width)];
      }
    }
